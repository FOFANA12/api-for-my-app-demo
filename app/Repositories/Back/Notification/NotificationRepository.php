...
namespace App\Repositories\Back\Notification;
...
class NotificationRepository
{
    public function getNotifications(Request $request)
    {
       ...
       /* $user = User::first();
        $message = new NotificationMessage('Admin','Test de notification','Ceci est un test de notification interne','success');
        ...
        if($filter == 'inbox'){
            $rows = DB::table('notifications_messages')
                ->join('users','notifications_messages.notifiable_id','=','users.id')
                ->select(
    ...
    public function getCounter(){
        ...
        $counter = DB::table('notifications_messages')
            ->join('users','notifications_messages.notifiable_id','=','users.id')
            ->select(
                DB::raw('(SELECT COUNT(id) FROM notifications_messages WHERE read_at IS NULL AND notifiable_id = users.id AND notifications_messages.deleted_at IS NULL) as unreadNotifications'),
                DB::raw('(SELECT COUNT(id) FROM notifications_messages WHERE favoris = true AND notifiable_id = users.id AND notifications_messages.deleted_at IS NULL) as favorisNotifications'),
                DB::raw('(SELECT COUNT(id) FROM notifications_messages WHERE notifications_messages.deleted_at IS NOT NULL AND notifiable_id = users.id) as trashedNotifications')
            )
            ->where('users.id','=',$currentUserId)
            ->first();
   ...
   /* public function getStatutNotifications()
    {
        $currentUserId = auth('user')->user()->id;
        ...
        $notifications = DB::table('notifications')
            ...
            ->select(
                DB::raw("notifications.id, type, notifiable_type, favoris, notifications.created_at, notifications.updated_at, notifications.deleted_at, notifiable_id, data, read_at")
            )
